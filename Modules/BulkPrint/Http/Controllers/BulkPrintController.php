...
class BulkPrintController extends Controller
{
    ...
    public function payrollBulkPrint(){
		...
		try{
			$roles = InfixRole::where('is_saas',0)->where('active_status', '=', '1')->where('id', '!=', 1)->where('id', '!=', 2)->where('id', '!=', 3)->where('id', '!=', 10)->where(function ($q) {
                $q->where('school_id', Auth::user()->school_id)->orWhere('type', 'System');
            })
			->orderBy('name','asc')
			->get();
    ...
    public function certificateBulkPrint(){
        try {
            $roles = InfixRole::where('is_saas',0)->where('id', '!=', 1)->Where('type', 'System')->get();
            $classes = SmClass::get();
    ...
    public function certificateBulkPrintSearch(Request $request)
    {
        try {
            ...
            $data['roles'] = InfixRole::where('is_saas',0)->where('id', '!=', 1)->Where('type', 'System')->get();
